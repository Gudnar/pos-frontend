<template>
  <div class="ide-view">
    ...
    <div v-else class="det-body">
      ...
      <!-- Sub-tabs -->
      <div class="ide-view__tabs" style="padding: 0 24px; border-bottom: 1px solid #f1f5f9; margin-bottom: 0;">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>
                ...
                <div v-if="item.productoId" style="font-size:11px;color:#64748b;">{{ categoriasDeProducto(item.productoId) }}</div>
              </td>
        ...
        <table v-else class="ide-table">
          ...
          <tbody>
            <tr v-for="p in pagos" :key="p.id">
              ...
              <td style="color:#64748b;">{{ fmtNum6(p.tipoCambio) }}</td>
          ...
          <tbody>
            <tr v-for="g in gastos" :key="g.id">
              <td>
                ...
                <div style="font-size:11px;color:#64748b;">{{ tipoGastoNombre(g.tipoGastoId) }}</div>
              </td>
              <td><span v-if="g.pais" class="ide-chip ide-chip--blue" style="font-size:10px;">{{ g.pais }}</span><span v-else style="color:#334155;">—</span></td>
              <td>{{ g.fechaGasto }}</td>
              <td>
                <span class="monto-con-moneda">{{ fmtNum(g.monto) }}</span>
                <span class="moneda-chip">{{ monedaNombre(g.monedaId) }}</span>
              </td>
              <td style="color:#64748b;">{{ fmtNum6(g.tipoCambio) }}</td>
      ...
      <div v-show="subTab === 'precio'" class="det-panel">
        ...
        <div class="pl-grid-2">
            ...
            <table v-else class="ide-table">
              ...
              <tbody>
                <tr v-for="row in pl_resumenPorPais" :key="row.pais">
                  ...
                  <td style="font-size:11px;color:#64748b;">{{ row.gastos.length }} gasto(s)</td>
              ...
              <tfoot>
                <tr>
                  <td colspan="3" style="text-align:right;font-size:11px;color:#64748b;padding:8px 12px;">Total logística (base):</td>
          ...
          <div class="pl-card">
            ...
            <table v-else class="ide-table">
              ...
              <tfoot>
                <tr>
                  <td colspan="2" style="text-align:right;font-size:11px;color:#64748b;padding:8px 12px;">Total compra (base):</td>
            ...
            <div class="pl-totales">
              ...
              <div class="pl-total-row pl-total-row--grand"><span>Gran Total</span><strong style="color:#e2e8f0;">{{ fmtNum(pl_totalLogBase + pl_totalCompraBase) }}</strong></div>
            </div>
          </div>
        </div>
        ...
        <div class="pl-card" style="margin-top:16px;">
          <div class="pl-card__title">
            ...
            <span style="font-size:10px;font-weight:400;color:#475569;"> — desactiva los que no afectan al precio de venta</span>
          </div>
          <table class="ide-table fb-gastos-table">
            ...
            <tbody>
              <tr v-for="g in gastos" :key="g.id" :class="{ 'fb-gasto-off': pl_gastosMap[g.id] === false }">
                ...
                <td>
                  <div>{{ g.descripcion }}</div>
                  <div style="font-size:11px;color:#475569;">{{ tipoGastoNombre(g.tipoGastoId) }}</div>
                </td>
                <td><span v-if="g.pais" class="ide-chip ide-chip--blue" style="font-size:10px;">{{ g.pais }}</span><span v-else style="color:#334155;">—</span></td>
                <td style="text-align:right;">{{ fmtNum(g.monto) }}</td>
                <td style="text-align:right;">{{ monedaNombre(g.monedaId) }}</td>
                <td style="text-align:right;color:#64748b;">{{ fmtNum6(g.tipoCambio) }}</td>
                ...
                <td style="text-align:right;font-weight:600;" :style="{ color: pl_gastosMap[g.id] === false ? '#334155' : '#94a3b8' }">
            ...
            <tfoot>
              <tr>
                <td colspan="7" style="text-align:right;font-size:11px;color:#64748b;padding:8px 12px;">Total gastos para precio (base):</td>
        ...
        <div class="ide-modal__body">
          ...
          <div class="fb-section">
            <div class="fb-section__title">
              ...
              <span style="font-size:10px;font-weight:400;color:#475569;"> — selecciona cuáles afectan el precio propuesto</span>
            </div>
            <div v-if="!gastos.length" style="font-size:12px;color:#64748b;">Sin gastos registrados.</div>
            <table v-else class="ide-table fb-gastos-table">
              ...
              <tbody>
                <tr v-for="g in gastos" :key="g.id" :class="{ 'fb-gasto-off': gastosParaPrecioMap[g.id] === false }">
                  ...
                  <td>
                    <div style="font-size:13px;color:#e2e8f0;">{{ g.descripcion }}</div>
                    <div style="font-size:11px;color:#475569;">{{ tipoGastoNombre(g.tipoGastoId) }}</div>
                  </td>
                  <td style="text-align:right;">{{ fmtNum(g.monto) }} <span style="font-size:10px;color:#475569;">{{ monedaNombre(g.monedaId) }}</span></td>
                  <td style="text-align:right;color:#64748b;">{{ fmtNum6(g.tipoCambio) }}</td>
                  <td style="text-align:right;">
                    <input
                  ...
                  <td style="text-align:right;font-weight:600;" :style="{ color: gastosParaPrecioMap[g.id] === false ? '#334155' : '#94a3b8' }">
              ...
              <tfoot>
                <tr>
                  <td colspan="5" style="text-align:right;font-size:11px;color:#64748b;padding:8px 12px;">Total gastos para precio (base):</td>
          ...
          <div class="fb-section">
            ...
            <div v-if="!formulaPasos.length" style="font-size:12px;color:#64748b;padding:8px 0;">
          ...
          <div class="fb-section">
            ...
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
              ...
              <div v-if="formulaRedondeo.tipo === 'multiplo'" style="display:flex;align-items:center;gap:8px;">
                <span style="font-size:12px;color:#64748b;">Múltiplo:</span>
          ...
          <div class="fb-section">
            <div class="fb-section__title">
              Vista previa por producto
              <span style="font-size:10px;font-weight:400;color:#475569;"> — actualiza en tiempo real</span>
            </div>
            <div v-if="!itemsConCosto.length" style="font-size:12px;color:#64748b;padding:8px 0;">
            ...
            <table v-else class="ide-table" style="margin-top:4px;">
              ...
              <tbody>
                <tr v-for="row in previewItemsConPrecio" :key="row.id">
                  ...
                  <td style="text-align:right;color:#64748b;">{{ fmtNum(row.costoTotalUnitario) }}</td>
          ...
          <div class="cierre-inventario-toggle" @click="ingresarInventario = !ingresarInventario">
            ...
            <div>
              <div style="font-size:13px;font-weight:600;color:#e2e8f0;">Ingresar al inventario automáticamente</div>
              <div style="font-size:11px;color:#64748b;margin-top:2px;">Crea un lote por cada producto vinculado al catálogo</div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async cargarTodo() {
      ...
      try {
        const [detalle, monedasData, tiposData, productosData, categoriasData, subcategoriasData, paisesData, fuentesData] = await Promise.all([
          ...
          this.$service.list('categorias-producto'),
          this.$service.list('subcategorias-producto'),
    ...
    async abrirCierreDialog() {
      ...
      // Inicializar todos los gastos como seleccionados con TC vacío (usa el original)
      const mapaGastos = {}
      const mapaTC = {}
      for (const g of this.gastos) {
    ...
    async cerrarOrden() {
      ...
      this.cerrando = true
      try {
        // Gastos excluidos (los que tienen false en el mapa)
        const gastosSeleccionados = this.gastos
          .filter(g => this.gastosParaPrecioMap[g.id] !== false)
          .map(g => g.id)
        const gastosExcluidos = this.gastos.length !== gastosSeleccionados.length
        // TC overrides (solo los que tienen valor distinto del original)
        const tcOverrides = this.gastos
          .filter(g => {
            const v = this.tcOverridesMap[g.id]
            return v !== '' && v != null && !isNaN(Number(v)) && Number(v) !== Number(g.tipoCambio)
...
.det-header { display: flex; align-items: center; gap: 12px; padding: 16px 24px; border-bottom: 1px solid #1e3a5f44; flex-wrap: wrap; }
.det-header__info { display: flex; align-items: center; gap: 10px; flex: 1; flex-wrap: wrap; }
.det-header__info h2 { font-size: 16px; font-weight: 700; margin: 0; color: #f1f5f9; }
...
.det-summary { display: flex; gap: 8px; font-size: 13px; color: #475569; align-items: center; flex-wrap: wrap; }
...
.ide-tab { background: none; border: none; padding: 10px 16px; border-radius: 0; cursor: pointer; font-size: 13px; color: #64748b; border-bottom: 2px solid transparent; transition: all .15s; display: flex; align-items: center; gap: 6px; }
.ide-tab--active { color: #818cf8; border-bottom-color: #6366f1; font-weight: 600; }
.tab-count { background: #1e3a5f44; color: #64748b; padding: 1px 6px; border-radius: 999px; font-size: 11px; font-weight: 600; }
...
.traz-card { background: #0d1526; border: 1px solid #1e3a5f44; border-radius: 10px; padding: 20px; }
.traz-card h4 { font-size: 11px; font-weight: 700; color: #475569; margin: 0 0 16px; text-transform: uppercase; letter-spacing: .5px; }
.traz-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 12px; }
.traz-kpi { background: #111d35; border: 1px solid #1e3a5f44; border-radius: 8px; padding: 12px 16px; }
.traz-kpi--highlight { background: #6366f111; border-color: #6366f133; }
.traz-kpi__label { font-size: 11px; color: #475569; margin-bottom: 4px; }
.traz-kpi__val { font-size: 18px; font-weight: 700; color: #e2e8f0; }
...
.fb-section__title { font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 10px; }
.fb-base-opts { display: flex; gap: 10px; }
.fb-base-opt {
  flex: 1; padding: 10px 14px; border-radius: 10px; cursor: pointer;
  border: 1px solid #1e3a5f55; background: #0a0f1e; transition: all .15s;
}
.fb-base-opt:hover { border-color: #6366f155; }
.fb-base-opt--active { border-color: #6366f1; background: #6366f111; }
.fb-base-opt__name { font-size: 13px; font-weight: 600; color: #e2e8f0; margin-bottom: 2px; }
.fb-base-opt--active .fb-base-opt__name { color: #818cf8; }
.fb-base-opt__desc { font-size: 11px; color: #475569; }
.fb-paso {
  display: flex; align-items: center; gap: 8px; margin-bottom: 8px;
}
.fb-paso__num { font-size: 11px; font-weight: 700; color: #475569; min-width: 18px; text-align: center; }
...
.fb-gastos-table tfoot td { background: #111d35; font-size: 12px; }
...
.pl-card {
  background: #0d1526; border: 1px solid #1e3a5f55; border-radius: 10px; padding: 18px 20px;
}
.pl-card__title {
...
.pl-totales { margin-top: 14px; border-top: 1px solid #1e3a5f44; padding-top: 10px; display: flex; flex-direction: column; gap: 6px; }
.pl-total-row { display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #64748b; padding: 2px 0; }
.pl-total-row strong { font-size: 14px; }
.pl-total-row--grand { border-top: 1px solid #1e3a5f55; margin-top: 4px; padding-top: 8px; color: #94a3b8; font-weight: 600; }
.pl-total-row--grand strong { font-size: 16px; }

.pl-formula-grid {
  display: grid; grid-template-columns: 1fr auto 1fr auto; gap: 12px; align-items: start;
  background: #111d35; border: 1px solid #1e3a5f44; border-radius: 10px; padding: 16px; margin-bottom: 14px;
}
@media (max-width: 1000px) { .pl-formula-grid { grid-template-columns: 1fr; } }

.pl-formula-bloque {
  background: #0a0f1e; border: 1px solid #1e3a5f44; border-radius: 8px; padding: 14px;
}
.pl-formula-bloque__title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; margin-bottom: 2px; }
.pl-formula-bloque__desc { font-size: 11px; color: #475569; margin-bottom: 12px; }
...
.pl-formula-label { font-size: 11px; color: #64748b; min-width: 120px; flex-shrink: 0; }
.pl-formula-input { width: 110px !important; flex-shrink: 0; }
.pl-formula-hint { font-size: 11px; color: #334155; white-space: nowrap; }
...
.cierre-inventario-toggle {
  ...
  background: #111d35; border: 1px solid #1e3a5f55; border-radius: 10px;
  cursor: pointer; transition: border-color .15s;
}
.cierre-inventario-toggle:hover { border-color: #22c55e55; }
.cierre-toggle-check {
  width: 20px; height: 20px; border-radius: 6px; flex-shrink: 0;
  border: 2px solid #334155; background: #0a0f1e;
  display: flex; align-items: center; justify-content: center;
...
.tc-doble-trigger {
  margin-top: 6px; font-size: 11px; color: #334155; cursor: pointer;
...
.tc-doble-hint { font-size: 10px; color: #475569; font-weight: 700; text-transform: uppercase; letter-spacing: .3px; }
.tc-doble-input { width: 118px !important; padding: 6px 8px !important; font-size: 12px !important; }
.tc-doble-op { font-size: 16px; color: #334155; padding-bottom: 2px; flex-shrink: 0; }
.tc-doble-result {
...
.pago-estado__label { font-size: 10px; color: #475569; text-transform: uppercase; letter-spacing: .4px; font-weight: 700; }
.pago-estado__val { font-size: 14px; font-weight: 700; color: #94a3b8; }
.pago-estado__val--pagado { color: #4ade80; }
.pago-estado__val--pendiente { color: #fb923c; }
.pago-estado__val--saldado { color: #4ade80; }
.pago-estado__sep { color: #1e3a5f; font-size: 18px; align-self: center; }

/* ── Barra de progreso de pago ───────────────────────────────────────── */
.pago-barra {
  width: 140px; height: 6px; border-radius: 999px;
  background: #1e3a5f44; overflow: hidden; align-self: center; flex-shrink: 0;
  display: flex;
}
...
.pago-dialog-resumen__label { font-size: 10px; color: #475569; text-transform: uppercase; letter-spacing: .4px; font-weight: 700; }
.pago-dialog-resumen__val { font-size: 15px; font-weight: 800; color: #94a3b8; }
.pago-dialog-resumen__val--ok { color: #4ade80; }
.pago-dialog-resumen__val--warn { color: #fb923c; }

/* ── Moneda inline chip en tabla ─────────────────────────────────────── */
.monto-con-moneda { font-weight: 600; color: #e2e8f0; }
.moneda-chip {
  display: inline-block; margin-left: 5px;
  padding: 1px 6px; border-radius: 5px; font-size: 10px; font-weight: 700;
  background: #1e3a5f55; color: #60a5fa; border: 1px solid #3b82f633;
...
.monedas-breakdown__label { font-size: 10px; color: #334155; text-transform: uppercase; letter-spacing: .3px; }
.moneda-breakdown-chip {
  padding: 2px 8px; border-radius: 6px; font-size: 11px; font-weight: 700;
  background: #1e3a5f44; color: #94a3b8; border: 1px solid #1e3a5f66;
}
...
.preview-moneda { font-size: 13px; color: #e2e8f0; font-weight: 600; }
.preview-cod {
...
.preview-op { color: #475569; font-size: 12px; }
.preview-base-label { font-size: 10px; color: #334155; }
</style>
