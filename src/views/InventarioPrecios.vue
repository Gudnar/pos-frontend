      ...
      <table v-else class="inv-table">
        ...
        <tbody>
          <tr v-for="p in precios" :key="p.id" :class="['inv-tr', p.activo ? '' : 'inv-tr--inactivo']">
            <td style="font-weight:600;color:#e2e8f0;">{{ p.productoNombre || '—' }}</td>
            <td><span class="inv-tipo-precio">{{ p.tipo }}</span></td>
            <td style="text-align:right;font-weight:800;color:#818cf8;">Bs {{ fmt(p.precio) }}</td>
            <td style="color:#64748b;">{{ p.moneda }}</td>
            <td style="color:#94a3b8;font-size:11px;">{{ p.fechaVigencia || '—' }}</td>
            <td style="color:#94a3b8;font-size:11px;">{{ p.fechaFin || '—' }}</td>
            <td style="text-align:right;color:#64748b;">{{ p.cantidadMin }}</td>
            <td style="text-align:right;color:#64748b;">{{ p.cantidadMax || '—' }}</td>
            <td style="text-align:center;">
              <span :class="p.activo ? 'inv-dot--on' : 'inv-dot--off'"></span>
            </td>
            <td style="color:#64748b;font-size:11px;">{{ formatFecha(p.fechaCreacion) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
.inv-dot--off { display:inline-block; width:8px; height:8px; border-radius:50%; background:#334155; }
