  ...
  <div style="height:100%;overflow-y:auto;padding:24px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
      <div>
        ...
        <p class="section-subtitle">Gestiona tus proveedores y sus datos de contacto</p>
      </div>
      <button v-if="clienteId" class="ct-new-btn" @click="abrirDialog()">+ Nuevo Proveedor</button>
    </div>
      ...
      <div v-for="item in items" :key="item.id" class="ide-ia-card ide-ia-card--hover">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div class="ct-avatar" :style="`background:${item.color || '#6366f1'}22;color:${item.color || '#6366f1'}`">
        ...
        <div v-if="item.nit" class="ct-info-row">
    ...
    <representantes-modal
      ...
      :endpoint="selectedItem ? `proveedores/${selectedItem.id}/representantes` : 'proveedores'"
      @changed="cargar"
    />
          ...
          <div class="ct-modal-body">
            <div class="ct-form-grid">
              <div class="ide-field">
                ...
                <input v-model="form.nombre" class="ide-input" placeholder="Nombre del proveedor o razón social" />
              </div>
              <div class="ide-field">
              ...
              <div class="ide-field">
                <label>NIT / RUC</label>
                <input v-model="form.nit" class="ide-input" placeholder="Número de identificación tributaria" />
              </div>
              ...
              <div class="ide-field">
                ...
                <input v-model="form.color" type="color" class="ide-input" style="height:38px;padding:2px 6px;cursor:pointer;" />
              </div>
              <div class="ide-field ct-form-full">
                <label>Notas</label>
...
const emptyForm = () => ({
  nombre: '', empresa: '', nit: '', categoria: '',
  direccion: '', notas: '', estado: 'activo', color: '#6366f1',
});
...
export default {
  ...
  data: () => ({
    ...
    editando: null,
    form: emptyForm(),
    categorias: ['Tecnología', 'Servicios', 'Materiales', 'Logística', 'Marketing', 'Consultoría', 'Otro'],
    repModal: false,
    selectedItem: null,
  }),
  computed: {
  ...
  methods: {
    async cargar() {
      try {
        this.loading = true;
        this.items = await this.$service.list('proveedores') || [];
      } catch {
        this.items = [];
      } finally {
        this.loading = false;
    ...
    async guardar() {
      ...
      try {
        if (this.editando) {
          await this.$service.put(`proveedores/${this.editando.id}`, this.form);
          this.$message.success('Proveedor actualizado');
        } else {
          await this.$service.post('proveedores', this.form);
          this.$message.success('Proveedor creado correctamente');
        }
        this.dialog = false;
        await this.cargar();
      } catch (e) {
        this.$message.error(e?.mensaje || e?.message || 'Error al guardar proveedor');
      } finally {
        this.saving = false;
      }
    },
    ...
    confirmarEliminar(item) {
      this.$confirm(`¿Eliminar al proveedor "${item.nombre}"?`, async () => {
        try {
          await this.$service.delete(`proveedores/${item.id}`);
          this.$message.success('Proveedor eliminado');
          await this.cargar();
        } catch (e) {
          this.$message.error(e?.mensaje || e?.message || 'Error al eliminar proveedor');
        }
      });
    },
  },
